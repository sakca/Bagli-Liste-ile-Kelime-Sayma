...
void degerara_ekle()
{
    ...
    while(iter!=NULL || s==-1) //i==-1 durumu, son kelimenin istisnai durumunda gerceklesir, istisnada iter=null durumdadir
    {
        ...
        sayac=0;
        iter=iter->sonraki;
        //aranan son kelimede bir istisna oldu, aranan kelime yazdırılıyor ancak üst satirda tanımlanan iter=null oluyor
        //bu istisnayi gidermek için döngünün +1 defa (son kelime icin) calisması gerekiyor
...
void liste_hazirla(char * ekle, int tane)
{
    ...
    else
    {
        ...
        else  //tane<ilkiTut iken
        {
            ...
            {
                while (iter2->sonraki->sonraki!=NULL)  //iter2->sonraki->sonraki== NULL ise, x1 x2 x3 x4 x5, x4u tutar //x4'un sonrası= x5, sonrası= null //bana x5 lazim // bu da iter2->sonraki eder
                {
